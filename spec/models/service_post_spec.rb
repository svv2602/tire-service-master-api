...
RSpec.describe ServicePost, type: :model do
  ...
  describe 'валидации' do
    ...
    describe 'post_number' do
      ...
      it 'должен быть положительным числом' do
        ...
        expect(service_post).not_to be_valid
        expect(service_post.errors[:post_number]).to include('Номер поста должен быть положительным числом')
    ...
    describe 'slot_duration' do
      ...
      it 'должна быть положительным числом' do
        ...
        expect(service_post).not_to be_valid
        expect(service_post.errors[:slot_duration]).to include('Длительность слота должна быть от 15 минут до 8 часов')
  ...
  describe 'скоупы' do
    ...
    describe '.ordered_by_post_number' do
      let(:unique_service_point) { create(:service_point) }
      let!(:post_3) { create(:service_post, service_point: unique_service_point, post_number: 3) }
      let!(:post_1) { create(:service_post, service_point: unique_service_point, post_number: 1) }
      let!(:post_2) { create(:service_post, service_point: unique_service_point, post_number: 2) }

      it 'возвращает посты отсортированные по номеру' do
        ordered_posts = unique_service_point.service_posts.ordered_by_post_number
