...
puts "🧪 Исправленное тестирование создания заказа..."
...
total_amount = order.order_items.sum(&:sum)
...
puts "  Общее количество: #{order.total_quantity}"
...
if order.save
  ...
  # Проверяем через API
  puts "\n🔍 Проверка через API:"
  api_order = Order.includes(:order_items).find(order.id)
  puts "  API заказ ID: #{api_order.id}"
  puts "  API товаров: #{api_order.order_items.count}"
else
