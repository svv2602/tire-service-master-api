puts "🆕 Создание расширенных email шаблонов..."
puts "========================================"

# Данные новых шаблонов
extended_templates = [
...
# Создание шаблонов
created_count = 0
extended_templates.each do |template_data|
  ...
  if template.persisted? && template.created_at > 1.minute.ago
    ...
    puts "ℹ️ Шаблон уже существует: #{template.name} (#{template.template_type})"
  end
end
...
puts ""
puts "✅ Новые типы уведомлений готовы к использованию:"
puts "   - booking_time_changed (изменение времени/даты)"
puts "   - booking_location_changed (изменение сервисной точки)"  
puts "   - booking_client_info_changed (изменение данных клиента)" 